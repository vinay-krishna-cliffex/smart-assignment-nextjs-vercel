...
class Home extends Component {
    ...
    componentDidMount() {
        ...
        // console.log("componentDidMount router", Router)
        ...
        // let valuesTesting = ''
        // valuesTesting = JSON.parse(localStorage.getItem('homeQuerys'));
        // console.log("valuesTesting", valuesTesting);
        // let windowValue = window.location.search.substring(1);
        // // windowValue = windowValue.replace("_", " ")
        // let string = windowValue
        // console.log("windowValue", windowValue,'hello');
        // let insuranceData = ''
        ...
        // if (valuesTesting && valuesTesting.insurance_provider && valuesTesting.insurance_provider !== insuranceData) {
        //     let data = { name: '', practice_name: '', insurance_provider: insuranceData, zip: '', consultAt: '' }
        ...
        if (values) {
            this.setState({ speciality: values.name, practiceName: values.practice_name, insurance_provider: values.insurance_provider, zip: values.zip, consult_at: values.consultAt }, () => {
                if (this.state.insurance_provider === 'cash pay') {
                    ...
                    !values.consultAt && this.handleGetZipDoctors(this.state.insurance_provider, this.state.speciality, this.state.zip, this.state.practiceName);
                }
            }
            );
        }
    ...
    handleSearchDoc = (e) => {
        ...
        const { insurance_provider, name, zip } = this.state;
        let values = JSON.parse(localStorage.getItem('homeQuerys'));
                                ...
                                <template>
                                    <Row className=" mb-4">
                                        <div className="col-12 d-flex justify-content-between align-items-center flex-column flex-md-row">
                                            <div className="mr-3">
                                                <p className="mb-3 mb-md-0"><strong className="mr-1">{this.state.docfilterData.length} </strong > Providers  &nbsp;&nbsp;</p>
                                            </div>
                                            <div style={{ display: 'flex', flexWrap: 'wrap', position: 'relative' }} className="align-items-center">
                                                <span>&nbsp;&nbsp; Sorted by: &nbsp;&nbsp;</span>
                                                {/* <button onClick={() => this.handleSortByZip} className="btn btn-sm btn-rounded mb0" style={{ outline: 'none' }}>Zip</button> */}
                                                <Button outline className="btn dropdown-toggle btn-selectpicker bs-placeholder" color="primary" style={{ letterSpacing: "1px", textTransform: "none" }}
                                                    onClick={this.handleSortByZip}>Zip</Button>
                                                {
                                                    this.state.isOpen &&
                                                    <div className="zipfilterdropDown dropdown-menu pb-0" >
                                                        <ul className="text-left p-0  inner overflow-auto mb-0" style={{ width: '100%', maxHeight: '350px' }}>
                                                            {
                                                                this.state.zipList.map((value, key) => {
                                                                    return <li key={key} className="dropdown-item">
                                                                        <div className="custom-control custom-checkbox">
                                                        ...
                                                        {/* <Button color="primary" className="w-100"
                                                            onClick={this.handleAppyZip}>APPLY</Button> */}
                                                        <div className="btn_apply shadow w-100 p-3">
                                                            <button className="text-white d-block border-0 focus_out" onClick={this.handleAppyZip}>APPLY</button>
                                                        </div>
